...
public class SportsFacilityService {
    ...
    @Cacheable(cacheNames = "facilityNear", key = "#root.args[0].generateCacheKey()")
    public List<FacilityNearResponse> getNear(FacilityNearRequest request) {
        ...
        return sportsFacilityRepository.findNear(
                        request.getLat(), request.getLng(), request.getRadiusM(), request.getLimit(), type
                ).stream()
                .map(FacilityMapper::toNearResponse)
                .toList();
    }

    @Cacheable(cacheNames = "facilityMarkers", key = "#root.args[0].generateCacheKey()")
